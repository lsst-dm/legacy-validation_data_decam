import lsst.obs.decam.processCcdDecam
assert type(config)==lsst.obs.decam.processCcdDecam.ProcessCcdDecamConfig, 'config is of type %s.%s instead of lsst.obs.decam.processCcdDecam.ProcessCcdDecamConfig' % (type(config).__module__, type(config).__name__)
# Perform ISR?
config.doIsr=False
...
# used in condition 3 for CR; see CR.cc code
config.calibrate.repair.cosmicray.cond3_fac2=0.6
